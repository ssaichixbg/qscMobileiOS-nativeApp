...
-(void)prepareLoadingToDisplay:(XYLoadingView*)entity
{
    ...
    if (entity.isLoading) {
        ...
        _alertView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"unexist.png"]];
    }
    _alertView.center = CGPointMake(screenBounds.size.width / 2, screenBounds.size.height / 2);
...
-(void)dismissLoadingView:(id)entity withSuccessfulMessage:(NSString*)message
{
    if(_alertViewQueue.count <= 0)
        return;
    ...
    if([entity isEqual:[_alertViewQueue lastObject]] && [entity isKindOfClass:[XYLoadingView class]])
    {
        ...
        [_alertViewQueue insertObject:loadingView atIndex:_alertViewQueue.count - 1];
        [self dismissLoadingViewWithAnimation:entity];
        [loadingView performSelector:@selector(dismiss) withObject:nil afterDelay:1.0f];
    }
}
